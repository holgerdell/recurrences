...
import { formatNumber, snapInt } from "./utils"
 ...
 */
export function formatPolynomialSystem(system: PolynomialSystem): string[] {
...
export function dominantRoot(system: PolynomialSystem, ACCEPTABLE_ERROR = 1e-8): Root | null {
	...
	for (const poly of sortedPolys) {
		...
		// --- 1D case ---
		// We assume there is exactly one root >= 1 for each variable.
		if (unknowns.length === 1) {
			const varName = unknowns[0]
			const f = (x: number): number => {
				const vals = { ...roots, [varName]: x }
				return evaluatePolynomial(poly, vals)
			}
			const val = findPositiveRoot(f, 1, 10)
