...
import { autogenerateRules, generateAllLocalSituations } from "./3coloring-rules"
import { analyzeRule } from "./rule-engine"
import { degreeFeatureProvider } from "./measure"

const ALL_LOCAL_SITUATIONS = Array.from(generateAllLocalSituations())
const AUTOGENERATED_RULES = ALL_LOCAL_SITUATIONS.map(autogenerateRules).map(r =>
	r.map(x =>
		analyzeRule(
			x,
...
const ALLOWED_COLORS: Set<number> = new Set([1, 2, 3, 4])

describe("3coloring-rules", () => {
	test("generated situations are unique", () => {
		// const signatures = ALL_LOCAL_SITUATIONS.map(s => s.signature)
		// expect(new Set(signatures).size).toBe(ALL_LOCAL_SITUATIONS.length)
		const situationIds = ALL_LOCAL_SITUATIONS.map(s => s.situationId)
		expect(new Set(situationIds).size).toBe(ALL_LOCAL_SITUATIONS.length)
	})
