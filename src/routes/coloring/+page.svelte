<script lang="ts">
	...
	const computeSituations = async () => {
		...
		for (const signature of enumerateSituations()) {
			const { situation, rules } = analyzeSituation(signature, counter)
	...
	const scalarRecurrences = $derived.by(() => {
		...
		return map2D(AUTOGENERATED_RULES, generatedGetRuleSolution)
	})
	...
	const handleStartSearch = () => {
		...
		lastBestBase = null
		weightSearchProgress = { mode: "Optimizing", percent: 0 }

		optimizationManager.start({
			ruleAnalyses: [...filteredRules],
