<script lang="ts">
	...
	import { readOrGenerate_ALL_LOCAL_SITUATIONS } from "$lib/coloring/parse-situations"
	import {
		analyzeRule,
	...
	const computeSituations = async () => {
		...
		for (const situation of readOrGenerate_ALL_LOCAL_SITUATIONS()) {
	...
	const displaySituations = $derived([
		...filteredLocalSituations.slice(0, 10),
		...filteredLocalSituations.slice(-10)
	])
	...
	let weightSearchProgress = $state({ total: 0, completed: 0, mode: "", percent: 0 })
	...
	const applyAutoHover = () => {
		...
		const j = autoHoverCell.w.coefficients["n_{2,3}"] * GRID_AXIS_COUNT
		const y = baseToIndex(base)
		hoveredCell = autoHoverCell
		drawHoverLine(j, y)
	}
	...
	const restartWeightSearch = () => {
		...
		let best: { weights: Measure | null; base: number } = {
			weights: null,
			base: Infinity
		}
		...
		worker.onmessage = (event: MessageEvent<GridWorkerMessage>) => {
			...
			if (message.type === "progress") {
				...
				const { cell } = message
				weightSearchProgress.completed = weightSearchProgress.completed + 1
				const base = cell.maxBase
				const i = baseToIndex(base)
				const j = !autoHoverCell ? 0 : autoHoverCell.w.coefficients["n_{2,3}"] * GRID_AXIS_COUNT
				weightGrid[j] = cell
		...
		const minPartialMeasure = fixedPartialMeasure
		const maxPartialMeasure = fixedPartialMeasure
		...
		<div class="mt-3 space-y-3">
			<div class="flex items-start gap-3">
				...
				<div class="space-y-0.5">
					<div class="font-semibold">Reading canonical local situations</div>
	...
	<div class="rounded-lg border border-amber-200 bg-white p-4 text-sm text-gray-800">
		...
		<div class="mt-4 space-y-3">
			...
			<div class="rounded-lg border border-gray-200 bg-gray-50 p-3 text-xs text-gray-800">
				{#if hoveredCell}
					<div class="space-y-2">
						<div class="text-sm font-semibold text-gray-600">Current best measure</div>
						<div class="font-mono text-sm text-gray-600">
							<BasicSubscripts raw={`μ = ${new Measure(hoveredCell.w.coefficients).toString()}`} />
						</div>
						<div class="text-lg">Solves to O({hoveredCell.maxBase.toFixed(4)}<sup>μ</sup>)</div>
					</div>
					{#if hoveredRule}
						<div class="flex flex-row gap-4">
							<div class="mt-3 space-y-1">
								<div class="text-sm font-semibold text-gray-600">
									Local situation #{hoveredCell.limitingSituationId}
								</div>
