<script lang="ts">
	...
	} from "$lib/coloring/feature-space"
	import { Graph } from "$lib/coloring/graph"
	import {
		analyzeRule,
		autogenerateRules,
	...
	const computeSituations = async () => {
		...
		let counter = 0
		for (const G of enumerateSituations()) {
			...
			// {
			// 	const edges = G.edges.concat([{ from: "a", to: "b" }])
			// 	const H = new Graph(
			// 		G.nodes.map(n => ({ ...n, halfedges: 0 })),
			// 		edges
			// 	)
			// 	const situation: LocalSituation = {
			// 		canon: H,
			// 		situationId: counter,
			// 		signature: (G.signature ?? "") + "_alt"
			// 	}
			// 	const rules = autogenerateRules(situation)
			// 	ALL_LOCAL_SITUATIONS.push(situation)
			// 	AUTOGENERATED_RULES.push(rules)
	...
	const analyzedRules = $derived(
		localSituationsStatus.loading
			? []
			: map2D(AUTOGENERATED_RULES, r => analyzeRule(r, computeFeatureVector, problem))
	)
	...
	const handleStartSearch = () => {
		...
		const ruleDeltasGroups = map2D(filteredRules, rule =>
			rule.branchDetails.map(b => features.map(f => b.featuresDelta[f] ?? 0))
		)
